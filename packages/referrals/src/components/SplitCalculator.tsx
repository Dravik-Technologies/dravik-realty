...
function InfoTooltip({ text }: { text: string }) {
  return (
    <span className="relative group inline-flex items-center cursor-help ml-1 align-middle">
      ...
      <span
        role="tooltip"
        className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 rounded-xl bg-dravik-dark px-3 py-2 text-[11px] leading-relaxed text-gray-200 shadow-xl text-center opacity-0 group-hover:opacity-100 transition-opacity z-20 whitespace-normal"
      >
        {text}
      </span>
    </span>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <div>
          <span className="text-sm font-semibold text-dravik-dark">{label}</span>
          {sublabel && (
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 flex items-start justify-between pb-3 min-w-0">
        <div className="min-w-0 pr-2">
          <div className="flex items-center text-[13px] font-medium text-dravik-dark leading-tight">
            {label}
          </div>
          {sublabel && (
        ...
        <span
          className={`text-[13px] font-bold flex-shrink-0 tabular-nums ${
            deduction ? "text-rose-600" : "text-dravik-dark"
          }`}
        >
          {deduction ? "−" : ""}
...
export default function SplitCalculator({ agent }: SplitCalculatorProps) {
  ...
  const breakdown = useMemo((): SplitBreakdown => {
    ...
    const dravikCut             = referringAgentGross * (companySplit / 100);
    const referringAgentNet   = referringAgentGross * (1 - companySplit / 100);
    return {
      totalCommission, receivingAgentShare, referringAgentGross, dravikCut, referringAgentNet,
    };
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-semibold text-dravik-dark block">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-7 pr-4 py-2.5 border border-line rounded-xl text-sm font-semibold text-dravik-dark bg-white focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition"
            />
          </div>
        </div>
      ...
      <div className="bg-surface-2 rounded-2xl p-5 space-y-5">
        ...
        <SliderRow
          ...
          sublabel="% of your referral share to Dravik Realty"
      ...
      <div className="flex items-center justify-between bg-dravik-dark rounded-2xl px-6 py-4">
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div>
          ...
          {/* Step 4 — Dravik deduction: minus sign + tooltip */}
          <FlowStep
            dotColor={RED_DOT}
            label={
              <>
                Dravik Realty Company Split
                <InfoTooltip text="Dravik Realty's standard company split on referred transactions." />
              </>
            }
            sublabel={`${companySplit}% of your gross share`}
            amount={breakdown.dravikCut}
            deduction
            hasLine={false}
          />
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[13px] font-bold text-dravik-dark">Your Net Payout</p>
                </div>
