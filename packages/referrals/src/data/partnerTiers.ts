...
export type PartnerTier = "App Subscriber" | "Preferred Agent";

const SUBSCRIBER_AGENT_IDS = new Set(["a1", "a2", "a3", "a4", "a5", "a6", "a7", "a8", "a14"]);

export function getPartnerTier(agent: Agent): PartnerTier {
  return SUBSCRIBER_AGENT_IDS.has(agent.id) ? "App Subscriber" : "Preferred Agent";
}
...
export function getPartnerTierLabel(tier: PartnerTier, compact = false): string {
  ...
  return compact ? "Preferred" : "Preferred Agent";
}
